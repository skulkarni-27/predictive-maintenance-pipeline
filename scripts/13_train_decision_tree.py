...
grid = GridSearchCV(model, {'max_depth': [5, 10, None]}, cv=3, scoring='f1', n_jobs=-1)
grid.fit(X_train, y_train)
best_model = grid.best_estimator_
y_pred = best_model.predict(X_test)
f1 = f1_score(y_test, y_pred)
os.makedirs('models', exist_ok=True)
joblib.dump(best_model, f"models/decisiontreeclassifier.pkl")
print(f"✓ DecisionTreeClassifier trained with F1={f1:.4f}")
